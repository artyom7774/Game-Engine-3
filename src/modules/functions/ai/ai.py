...
def compileProgramCode(text):
    ...
    def error(program):
        ...
        for id, node in program.items():
            for key, value in node["inputs"].items():
                if value["value"] is not None:
                    if str(value["value"]["id"]) not in program:
                        errors += f"\nnot found ID={value['value']['id']} in node={id}"
                        count += 1

                    if str(value["value"]["id"]) in program and value["value"]["name"] not in program[str(value["value"]["id"])]["outputs"]:
                        errors += f"\nnot found output name={value['value']['name']} in node={value['value']['id']} for node={id}"
                        count += 1
    ...
    for i, line in enumerate(text.split("\n")):
        try:
            ...
            errors += f"error on line={i + 1} {e}\n"
            count += 1

    temp = error(compile())

    return compile(), (temp[0] + count, errors + temp[1])
