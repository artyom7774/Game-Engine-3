...
def getNoiseValue(program, compiler, path: str, nodes: dict, id: int, variables: dict, **kwargs) -> dict:
    ...
    # Извлечение параметров из узлов
    ...
    random.seed(seed)
    x_offset = x + random.uniform(-0.1, 0.1)
    y_offset = y + random.uniform(-0.1, 0.1)
    ...
    if centered >= 0:
        stretched = centered ** (1.0 / amplitude)

    else:
        stretched = -((-centered) ** (1.0 / amplitude))

    normalized = (stretched / 2.0) + 0.5

    answer = mn + normalized * (mx - mn)
