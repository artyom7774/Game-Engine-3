...
class ObjectGroup:
    ...
    def add(self, obj: VObject) -> None:
        ...
        self.objectsById[str(obj.id)] = obj

        if obj.group not in self.objectsByGroup:
            self.objectsByGroup[obj.group] = {}

        self.objectsByGroup[obj.group][str(obj.id)] = obj
    ...
    def remove(self, obj: VObject) -> None:
        ...
        self.objectsById.pop(str(obj.id))

        self.objectsByGroup[obj.group].pop(str(obj.id))

    def getById(self, id: int) -> VObject:
        return self.objectsById.get(str(id))

    def removeByGroup(self, group: str) -> None:
        if group not in self.objectsByGroup:
            return

        for element in self.objectsByGroup[group].values():
            self.objects.remove(element)

        self.objectsByGroup[group] = {}

    def removeById(self, id: int) -> None:
        if str(id) in self.objectsById:
            self.remove(self.objectsById[str(id)])
