...
class CreateButtonFunctions:
    @staticmethod
    def create(project, dialog, event, name: str = None, logger: bool = True, loadFile: str = "engine/files/button.json", save: str = None) -> None:
        if save is None:
        ...
        if logger:
            if name == "":
                dialog.objects["log_label"].setText("Imposiable button name")
            ...
            for element in os.listdir(path):
                if element == name:
                    dialog.objects["log_label"].setText("Button name already exist")
        ...
        if name == "":
            ...
            with open(f"{path}/{name}.btn", "w", encoding="utf-8") as file:
                dump(out, file, indent=4)
...
class CreateButton(QDialog):
    def __init__(self, project, parent=None) -> None:
        QDialog.__init__(self, parent)

        self.project = project

        self.setWindowTitle(translate("Create button"))
    ...
    def init(self) -> None:
        ...
        self.objects["name_label"] = QLabel(parent=self, text=translate("Button name") + ":")
        ...
        self.objects["create_button"].clicked.connect(lambda event: CreateButtonFunctions.create(self.project, self, event))
